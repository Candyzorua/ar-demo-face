declare var WebARRocksFaceShape2DHelper: any;
declare var WebARRocksResizer: any;
...
export class LipstickComponent {
  private _canvasVideo: any = null
  private _canvasAR: any = null;
  
  // tweak contours coefficients - 0 -> no tweak:
  private mouthWiden: number = 0.01;
  private upperLipOut: number = 0;//0.01;
  private lowerLipOut: number = 0.005;//0.01;
  
  private SHAPELIPS: any = {
    name: 'LIPS',
    ...
    outlines: [
      { // upper lip. Indices of points in points array:
        ...
        displacements: [ // displacements, relative to perimeter:
          this.mouthWiden,
          this.upperLipOut, this.upperLipOut, this.upperLipOut - 0.015, this.upperLipOut, this.upperLipOut, // exterior
          0.00, 0,
      ...
      { // lower lip:
        ...
        displacements: [
          ...
          0.015, 0.02, 0.015,
          0, 0.0,
          this.lowerLipOut, this.lowerLipOut, this.lowerLipOut, this.lowerLipOut, this.lowerLipOut,
          0.0
        ]
      }
    ],
  ...
  constructor(
    ...
    this.vtoService.changeMode('lipstick');
  }
  ...
  // entry point:
  main(){
    this._canvasAR = document.getElementById('WebARRocksFaceCanvasAR');
    this._canvasVideo = document.getElementById('WebARRocksFaceCanvasVideo');
  
    WebARRocksResizer.size_canvas({
      canvas: this._canvasVideo,
      overlayCanvas: [this._canvasAR],
      callback: () => {
        WebARRocksFaceShape2DHelper.init({
          NNCPath: '../../neuralNets/NN_LIPS_8.json',
          canvasVideo: this._canvasVideo,
          canvasAR: this._canvasAR,
          shapes: [ this.SHAPELIPS ]
        }).then(function(){
